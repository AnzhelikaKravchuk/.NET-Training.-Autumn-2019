...
> Одномерный массив считать упорядоченным, если отношение порядка выполняется для элементов, индексы которых удовлетворяют некоторому заданному условию (например, диапазон и шаг изменеения, удоблетворение условия кратности и т.п.),  а само отношение порядка определяется некоторой функцией-ключем (например, определяющй количество заданного символа в p-ичном (2<=p<=16) числа,его модуль и т.п.).  
